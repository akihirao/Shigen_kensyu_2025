...
source <- "url" # local (default), url, or original
...
# ドリフトの加速度
accel_month <- diff(drift_ts)          # ℃ / 月^2
accel_yearscale <- accel_month * 144   # ℃ / 年^2

# 年平均したドリフトの加速度
drift_year <- aggregate(drift_ts, nfrequency = 1, FUN = mean) * 12
accel_year <- diff(drift_year)          # ℃ / 年^2

plot(drift_year,
     ylab = "Annualized slope (°C / year)",
     main = "Long-term change in SST trend")
abline(h = 0, lty = 2)
...
smooth_pred <- predict(fit_SST$model,
                       interval = "prediction",
                       levels= 0.95)

forecast_tidy <- rbind(smooth_pred,forecast_pred) %>%
  as_tibble() %>% 
  mutate(SST=c(as.numeric(SST_ts),rep(NA,6)),
          time=c(time(SST_ts),c(2026.000,2026.083,2026.167,2026.250,2026.333,2026.417)))

predict_plot <- ggplot(data=forecast_tidy,
                         aes(x=time,y=SST)) +
  labs(title="Level component",x="Year", y="SST") +
  geom_point(alpha = 0.5) +
  geom_line(aes(y=fit), size = 1.2) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), alpha = 0.3) +
  scale_x_continuous(limits = c(2024,max(forecast_tidy$time)))
